...
class {{cookiecutter.class_name}}(object):
  ...
  def {{cookiecutter.def_getData}}(self):
    #Getting raw data
    df = pd.read_csv( {{cookiecutter.csv_path}} )
    return df

  def {{cookiecutter.def_preprocess}}(self, df):
    #dropping irrelevant columns 'host_id', 'host_name'
    if {{cookiecutter.target}} in df.columns:
    ...
    # define our features 
    if {{cookiecutter.target}} in df.columns:
      features = df.drop([{{cookiecutter.target}}], axis=1)
    else: 
      features = df
    ...
    # define our target
    if {{cookiecutter.target}} in df.columns:
      target = df[[{{cookiecutter.target}}]]
      return features_recoded,target
    else:
      return features_recoded


  def {{cookiecutter.def_splitData}}(self, df, target):
    #return X_train, X_test, y_train, y_test
    return train_test_split(df, target, test_size=0.2)

  def {{cookiecutter.def_scaleDataTrain}}(self, X_train, X_test):
    # scale data
    X_train = self.sc.fit_transform(X_train)
    X_test  = self.sc.transform(X_test)
    return X_train, X_test

  def {{cookiecutter.def_scaleDataPredict}}(self, df):
    array = df.to_numpy()
    return array

  def {{cookiecutter.def_parameterOptimizer}}(self, X_train, y_train):
    # create Grid
    ...
    joblib.dump(booster_grid_search, {{cookiecutter.booster_grid_search}}, compress = 1) # Only best parameters

  def {{cookiecutter.def_train}}(self, X_train, y_train, X_test):
    # Load best parameters
    booster_params = joblib.load({{cookiecutter.booster_grid_search}})
  ...
  def {{cookiecutter.def_metrics}}(self, y_test, y_pred_test, X_train, y_train):
  ...
  def {{cookiecutter.def_pickleModel}}(self, model):
    #Saves the trained vectorizer for future use.

    path={{cookiecutter.path_model_pkl}}
